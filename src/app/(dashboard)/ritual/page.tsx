...
import { useStore, CATEGORY_LABELS, type GoalCategory } from '@/lib/store';
...
export default function WeeklyRitualPage() {
  ...
  const lastCompleted =
    typeof window !== 'undefined' ? localStorage.getItem('weeklyRitualLastCompleted') : null;

  // Last 7 days mood summary
  const moodSummary = useMemo(() => {
    const sevenDaysAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
    const recent = store.journalEntries.filter(
      (e) => e.mood && new Date(e.createdAt).getTime() > sevenDaysAgo
    );
    ...
    return top ? { mood: top[0], count: top[1], total: recent.length } : null;
  ...
  function handleComplete() {
    ...
    if (hasContent) {
      ...
      const reflectionItems = goalReflections
        .map(([goalId, note]) => {
          const goal = store.goals.find((g) => g.id === goalId);
          return `<li><strong>${goal?.title ?? goalId}:</strong> ${note}</li>`;
        })
        .join('');
      const priorityItems = priorityLines.map((p) => `<li>${p.title}</li>`).join('');
      const html = [
        `<h2>Weekly Ritual — ${dateStr}</h2>`,
        goalReflections.length > 0
          ? `<p><strong>Goal reflections:</strong></p><ul>${reflectionItems}</ul>`
          : '',
        priorityLines.length > 0
          ? `<p><strong>Top ${priorityLines.length} priorities this week:</strong></p><ol>${priorityItems}</ol>`
          : '',
      ]
        .filter(Boolean)
        .join('');
    ...
    // Persist completion date so dashboard can show "last completed"
    localStorage.setItem('weeklyRitualLastCompleted', new Date().toISOString());
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div
          className="px-10 py-8 border-b flex items-start justify-between shrink-0"
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              {moodSummary && (
                ...
                >
                  7-day mood: {moodSummary.mood} ({moodSummary.total} entries)
                </span>
              )}
            </div>
          </div>
        ...
        {/* Step tabs */}
        <div
          className="px-10 py-3 border-b flex items-center gap-8 shrink-0"
              ...
              {activeGoals.length === 0 ? (
                <div className="glass-static p-8 text-center">
                  <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                    No active goals. Create some first!
                  </p>
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setGoalNotes({ ...goalNotes, [goal.id]: e.target.value })
                          }
                          rows={2}
                    ...
                    <input
                      ...
                      placeholder={
                        idx === 0
                          ...
                          : idx === 1
                            ? 'e.g., Plan content for next week'
              ...
              <div className="glass-static p-5 mb-6">
                ...
                <div className="space-y-3">
                  ...
                  {priorities.every((p) => !p.title.trim()) && (
                    <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                      No priorities set — go back and add some.
                    </p>
                  )}
                </div>
              </div>
        ...
        {/* Footer */}
        <div
          className="px-10 py-6 border-t flex items-center justify-between shrink-0"
          style={{ borderColor: 'var(--border)' }}
        >
          <button
