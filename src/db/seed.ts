...
async function seed() {
  ...
  const [goalFitness] = await db
    ...
    .values({
      ...
      description: 'Train consistently and complete a half-marathon (21 km) within the year.',
      category: 'health',
      priority: 'high',
  ...
  await db.insert(schema.tasks).values([
    {
      ...
      description: 'Update /api/goals to use drizzle-orm instead of the client-side store.',
      status: 'todo',
      priority: 'critical',
      durationMinutes: 90,
  ...
  // ------------------------------------------------------------------
  console.log('  → Inserting chat sessions and messages…');

  const [sessionLotion] = await db
    .insert(schema.chatSessions)
    .values({
      userId,
      title: 'Am I over-engineering?',
      goalId: goalLotion.id,
      createdAt: new Date('2026-06-04T15:00:00Z'),
      updatedAt: new Date('2026-06-04T15:01:00Z'),
    })
    .returning();

  const [sessionFitness] = await db
    .insert(schema.chatSessions)
    .values({
      userId,
      title: 'Skipping morning runs',
      goalId: goalFitness.id,
